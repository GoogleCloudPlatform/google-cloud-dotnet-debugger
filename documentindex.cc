...
bool DocumentIndex::Initialize(PortablePdbFile *pdb, int doc_index) {
  ...
  for (size_t method_def = 1; method_def < num_of_methods; ++method_def) {
    ...
    if (debug_info_row.document != doc_index) continue;
...
bool DocumentIndex::ParseMethod(Method *method, PortablePdbFile *pdb,
  ...
  for (size_t index = 1; index < local_scope_table.size(); ++index) {
    ...
    method->local_scope.push_back(ParseScope(pdb,
      local_scope_row, local_scope_table, local_variable_table,
      local_constant_table, method_def, index));
  }

  return true;
}

DocumentIndex::Scope DocumentIndex::ParseScope(PortablePdbFile *pdb,
  ...
  if (scope_index + 1 < local_scope_table.size()) {
    ...
    LocalScopeRow next_scope_row = local_scope_table[scope_index + 1];
